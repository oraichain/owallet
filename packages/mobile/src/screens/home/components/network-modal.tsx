...
export const NetworkModal = ({ stakeable }) => {
  ...
  const getBalanceByChainId = (chainId, networkType, stakeCurrency, isAll) => {
    const chainBalance: PricePretty = new PricePretty(
      fiatCurrency,
      new Dec("0")
    );
    ...
    if (totalStake.isReady) {
      ...
      // ) {
      //   return chainBalance.add(stakeRes);
      // }
      if (isAll) {
  ...
  for (let i = 0; i < allChain.length; i++) {
    ...
    getBalanceByChainId(chainId, networkType, stakeCurrency, true);
  }
  const _renderItem = ({
    item,
  }: {
